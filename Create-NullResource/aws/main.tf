...
# -----------------------------
# null_resource to print integration and call the function via API Gateway
# -----------------------------
resource "null_resource" "call_api_gateway_function" {
  provisioner "local-exec" {
    command = <<EOT
    echo "Integration Details:"
    echo "API Gateway URL: ${aws_apigatewayv2_api.http_api.api_endpoint}"
    echo "Lambda ARN: ${aws_lambda_function.put_function.arn}"

    # Wait for 10 seconds before triggering the API
    echo "Waiting for 10 seconds before triggering the API..."
    sleep 15

    # Trigger the Lambda function via API Gateway
    curl -X PUT -H "Content-Type: application/json" \
    -d '{"id": 123212, "name": "Testing"}' \
    "${aws_apigatewayv2_api.http_api.api_endpoint}/"

    echo "API call to Lambda function completed!"
    EOT
  }
